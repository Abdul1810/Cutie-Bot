...
const {  sepia } = require('../api/Canvas');
...
class Sepia extends Command {
	...
	get description () {
		return 'To get user\'s avatar in sepia'
	}

	get usage () {
		return '{prefix}sepia  <@user>'
	}
	...
	async run (msg, args) {
		...
		if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}sepia  <@user>`
				}
			}
		})
