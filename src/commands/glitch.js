...
const { distort } = require('../api/Canvas');
...
class Glitch extends Command {
	...
	get description () {
		return `To get a user\'s avatar with glitch effect"`
	}

	get usage () {
		return '{prefix}glitch  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}glitch  <@user>`
				}
			}
		})
		...
		try {
			user = this.bot.users.get(memberID)
                ...
                distort(ctx, 20, 0, 0, ddata.width, ddata.height, 5);
