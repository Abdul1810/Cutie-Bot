...
const { contrast } = require('../api/Canvas');
...
class Contrast extends Command {
	...
	get description () {
		return 'To get user\'s avatar with contrast'
	}

	get usage () {
		return '{prefix}contrast  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}contrast  <@user>`
				}
			}
		})
		...
		try {
			user = this.bot.users.get(memberID)
                ...
                contrast(ctx, 0, 0, ddata.width, ddata.height);
