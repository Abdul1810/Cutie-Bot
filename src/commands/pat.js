...
const { GetPatGif } = require('../api').neko
...
class Pat extends Command {
	...
	get description () {
		return 'pat a User Virtually.'
	}

	get usage () {
		return '{prefix}pat <@user>'
	}
	...
	async run (msg) {
		if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}pat <@user> `
				}
			}
		})
			...
			embed:{
				...
				author:{
					name: `${msg.author.username}#${msg.author.discriminator} Pats ${msg.mentions[0].username}#${msg.mentions[0].discriminator}`,
