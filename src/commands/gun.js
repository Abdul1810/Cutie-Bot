...
class Gun extends Command {
	...
	get description () {
		return `To get gun over user\'s avatar.`
	}

	get usage () {
		return '{prefix}gun  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}gun  <@user>`
				}
			}
		})
		...
		try {
			user = this.bot.users.get(memberID)
				...
				let m = await msg.channel.createMessage('<a:emoji_40:798036610045444096> this might take few seconds');
                        ...
                        const ratio = (ddata.height / 2) / base.height;
                        const width = base.width * ratio;
                        ctx.drawImage(base, ddata.width - width, ddata.height - (ddata.height / 2), width, ddata.height / 2);
