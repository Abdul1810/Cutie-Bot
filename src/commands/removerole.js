...
class removerole extends Command {
	...
	get description () {
		return 'To remove role to mentioned user'
	}

	get usage () {
		return '{prefix}removerole <@user or userID> <@role or roleID> '
	}
	...
	async run (msg, args) {
		...
		if (!args[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}removerole <@user or userID> <@role or roleID>`
				}
			}
		})
		...
		try {
			return await member.removeRole(roleID, `${this.bot.sanitizer.sanitiz(msg.author.username)} Removed Role.`), msg.channel.createMessage({
				embed:{
					color: this.bot.color,
					description: `<a:Cutie_Verifiedtick:769893107830882344> **${msg.author.username}**, Removed <@&${roleID}> To ${member.username}`
				}
			})
		} catch (e) {
			return msg.channel.createMessage({
