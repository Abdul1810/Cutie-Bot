...
const {  invert } = require('../api/Canvas');
...
class Invert extends Command {
	...
	get description () {
		return 'To get user\'s avatar in invert.'
	}

	get usage () {
		return '{prefix}invert  <@user>'
	}
	...
	async run (msg, args) {
		...
		if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}invert  <@user>`
				}
			}
		})
