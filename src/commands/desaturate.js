...
const { desaturate } = require('../api/Canvas');

class Desaturate extends Command {
	get name () {
		return 'desaturate'
	}
	...
	get description () {
		return 'To get a user\'s avatar but it was desaturated.'
	}

	get usage () {
		return '{prefix}desaturate  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}desaturate  <@user>`
				}
			}
		})
