...
const dictionary = require('../assets/json/fancy.json');

class Fancy extends Command {
	get name () {
		return 'fancy'
	}
	...
	get description () {
		return 'To converts text to fancy letters'
	}

	get usage () {
		return '{prefix}fancy <text>'
	}
	...
	async run (msg, args) {
		...
		if (!args[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}fancy <text>`
				}
			}
		})
