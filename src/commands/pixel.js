...
const {  pixelize } = require('../api/Canvas');
...
class Pixel extends Command {
	...
	get description () {
		return 'To get a user\'s avatar pixelized.'
	}

	get usage () {
		return '{prefix}pixel  <@user>'
	}
	...
	async run (msg, args) {
		...
		if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}pixel  <@user>`
				}
			}
		})
