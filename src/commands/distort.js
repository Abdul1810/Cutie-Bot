...
class Distort extends Command {
	...
	get description () {
		return 'To get a user\'s avatar but it was distort.'
	}

	get usage () {
		return '{prefix}distort  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}distort  <@user>`
				}
			}
		})
