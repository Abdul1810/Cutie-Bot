...
class Heman extends Command {
	...
	get description () {
		return `Draws a user\'s avatar over He-Man\'s face.`
	}

	get usage () {
		return '{prefix}he-man  <@user>'
	}
	...
	async run (msg, args) {
        ...
        if (!msg.mentions[0]) return msg.channel.createMessage({
			embed:{
				...
				footer:{
					text: `Usage: ${await this.bot.mongo.getprefix(msg.channel.guild.id)}he-man  <@user>`
				}
			}
		})
